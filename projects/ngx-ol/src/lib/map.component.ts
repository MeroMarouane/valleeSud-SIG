...
export class MapComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  ngOnInit() {
    ...
    // Create a new map
    this.mapService.setMap(this.id, this.instance);
    ...
    this.instance.on('change:size', (event: ObjectEvent) =>
      this.olChangeSize.emit(event)
    );
    this.instance.on('change:target', (event: ObjectEvent) =>
      this.olChangeTarget.emit(event)
    );
    this.instance.on('change:view', (event: ObjectEvent) =>
      this.olChangeView.emit(event)
    );
    ...
    this.instance.on('postcompose', (event: RenderEvent) =>
      this.olPostCompose.emit(event)
    );
    this.instance.on('postrender', (event: RenderEvent) =>
      this.olPostRender.emit(event)
    );
    this.instance.on('postrender', (event: MapEvent) =>
      this.postRender.emit(event)
    );
    this.instance.on('precompose', (event: RenderEvent) =>
      this.olPreCompose.emit(event)
    );
    this.instance.on('propertychange', (event: ObjectEvent) =>
      this.olPropertyChange.emit(event)
    );
  ...
  ngOnDestroy(): void {
    this.mapService.remove(this.id);
  }
}
